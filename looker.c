...
/*   Updated: 2022/08/16 15:07:46 by llord            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{						// Finds the closest value from a given value range in the stack
	int	distance_down;
	int	distance_up;

	if (range_min <= (*stack).list[(*stack).pos] && \
	(*stack).list[(*stack).pos] <= range_max)
		return (0);
